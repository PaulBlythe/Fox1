...
{
    public class LoadObject : SubScene
    {
        ...
        public override void Update(float dt)
        {
            ...
            if (button.Contains(ms.X, ms.Y))
            {
                if ((ms.LeftButton == ButtonState.Released) && (old.LeftButton == ButtonState.Pressed))
                {
                    if (fb == null)
                    {
                        fb = new DebugFileBrowser(FilePaths.DataPath, "*.gameobject");
                    }
                }
            }
            old = ms;
        ...
        public override void Draw()
        {
            ...
            else
            {
                ...
                batch.DrawString(AssetManager.GetDebugFont(), "Load Object", new Vector2(20, 18), Color.White);
