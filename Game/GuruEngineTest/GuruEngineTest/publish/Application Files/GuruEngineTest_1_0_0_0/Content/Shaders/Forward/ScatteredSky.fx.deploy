...
vertexOutput mainVS (appdata IN)
{
	...
	OUT.UV = IN.UV0;
	OUT.MoonDirection = IN.Position.xyz - MoonPosition;
	return OUT;
}
...
{	
	...
	float3 moonvec = normalize(-IN.MoonDirection);
	...
	float moonHighlight = pow(max(0, dot(moonvec, -eyeVec)), sunRadiusAttenuation) * SunLightness;
	...
	// Add sun highlights
	fogColor += sunHighlight + largeSunHighlight + moonHighlight;
