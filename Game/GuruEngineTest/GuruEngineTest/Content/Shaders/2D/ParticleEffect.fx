...
float4 ComputeParticlePosition(float3 position, float3 velocity, float age, float normalizedAge)
{
    ...
    // Work out how fast the particle should be moving at the end of its life, by applying a constant scaling factor to its starting velocity.
    float endVelocity = startVelocity * EndVelocity;
    
    // Our particles have constant acceleration, so given a starting velocity S and ending velocity E, at time T their velocity should be S + (E-S)*T.
    // The particle position is the sum of this velocity over the range 0 to T.
    // To compute the position directly, we must integrate the velocity equation. 
	// Integrating S + (E-S)*T for T produces S*T + (E-S)*T*T/2.
...
float4 ComputeParticleColor(float4 projectedPosition, float randomValue, float normalizedAge)
{
    ...
    // Fade the alpha based on the age of the particle. 
	// This curve is hard coded to make the particle fade in fairly quickly, then fade out more slowly:
    //   plot x*(1-x)*(1-x) for x=0:1 in a graphing program if you want to see what this looks like. 
...
VertexShaderOutput ParticleVertexShader(VertexShaderInput input)
{
	...
	if (texture_animation_scale > 1)
	{
		int phase = normalizedAge * texture_animation_scale;
		phase = min(phase, texture_animation_scale - 1);

		float scale = (1.0 / texture_animation_scale);
		float offset = phase * scale;
		float cor = (output.TextureCoordinate.x * scale);
		output.TextureCoordinate.x =  offset + cor;
	}
