...
namespace GuruEngine.Rendering.Particles
{
    ...
    public class ParticleSystem
    {
        ...
        /// Constructor.
        /// </summary>
        public ParticleSystem(GraphicsDevice device, ParticleSettings set)
        {
            this.settings = set;
            this.device = device;
        ...
        {
            LoadParticleEffect();
            String file = FilePaths.DataPath + @"Textures\Particles\" + settings.TextureName;
        ...
        void LoadParticleEffect()
        {
            ...
            particleEffect = AssetManager.GetCloneOfEffect(@"Shaders\2D\ParticleEffect".GetHashCode());
            ...
            parameters["texture_animation_scale"].SetValue(settings.TextureCells);
        }
        ...
        public void Update(GameTime gameTime)
        {
            ...
            String file = FilePaths.DataPath + @"Textures\Particles\" + settings.TextureName;
            int guid = file.GetHashCode();
            if (AssetManager.IsReady(guid))
            {
                Texture2D tex = AssetManager.Texture(guid);
                particleEffect.Parameters["Texture"].SetValue(tex);
                ready = true;
            }
        ...
        public void Draw(GameTime gameTime, Matrix view, Matrix projection)
        {
            ...
            if (firstActiveParticle != firstFreeParticle)
            {
                ...
                // Set an effect parameter describing the viewport size. 
                // This is needed to convert particle sizes into screen space point sizes.
                effectViewportScaleParameter.SetValue(new Vector2(0.5f / device.Viewport.AspectRatio, -0.5f));

                // Set an effect parameter describing the current time. 
                // All the vertex shader particle animation is keyed off this value.
                ...
                foreach (EffectPass pass in particleEffect.CurrentTechnique.Passes)
                {
                    ...
                    if (firstActiveParticle < firstFreeParticle)
                    {
                        ...
                        device.DrawIndexedPrimitives(PrimitiveType.TriangleList, 0, firstActiveParticle * 6, (firstFreeParticle - firstActiveParticle) * 4);
                    }
                    else
                    {
                        // If the active particle range wraps past the end of the queue
                        // back to the start, we must split them over two draw calls.
                        device.DrawIndexedPrimitives(PrimitiveType.TriangleList, 0, firstActiveParticle * 6, (settings.MaxParticles - firstActiveParticle) * 4);

                        if (firstFreeParticle > 0)
                        {
                            device.DrawIndexedPrimitives(PrimitiveType.TriangleList, 0, 0, firstFreeParticle * 6);
                        }
                    }
                }
        ...
        public void AddParticle(Vector3 position, Vector3 velocity)
        {
            ...
            // Adjust the input velocity based on how much this particle system wants to be affected by it.
            ...
            // Choose four random control values. 
            // These will be used by the vertex shader to give each particle a different size, rotation, and color.
