...
namespace GUITestbed.GUI.Dialogs
{
    class OpenFileDialog : Dialog
    {
        ...
        public OpenFileDialog(String path, String title, String ext, Rectangle r, CallBack caller)
            : base(title, r)
        {
            SearchPath = path;
            ...
            b2 = new Button(new Rectangle(r.X + r.Width - 130, r.Y + r.Height - 40, 120, 30), "Open");
            b2.Parent = this;
            Children.Add(b2);
        ...
        public override void Message(string s)
        {
            ...
            if (s.Contains("["))
            {
                int c1 = s.IndexOf("[") + 1;
                int c2 = s.IndexOf("]") - 1 ;
                String ss = s.Substring(c1, c2 - c1);
                ss = ss.TrimStart(' ');
                ss = ss.TrimEnd(' ');
                SearchPath = Path.Combine(SearchPath, ss);
                Search();
            }
